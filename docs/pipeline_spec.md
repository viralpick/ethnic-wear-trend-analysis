...
**Partial-share representative 적재 정책 (실 동작)**:
- `aggregation.representative_builder.build_contributions` 는 현재 **N=3 (multiplier=5.0)** 만 emit.
- N=1/2 multiplier (1.0/2.5) 는 `multiplier_for_n` 함수로 미리 구현됐으나 호출 X (정책 미진입).
- cross-product 결과 다수 representative_key 는 모두 N=3 동일하지만, summary (트렌드 요약) 매칭은
  cluster_key ↔ representative_key top-1 만. summary 미매칭 representative 는 score_total / lifecycle /
  weekly_change_pct / score_breakdown 모두 NULL, **`total_item_contribution` + `factor_contribution`
  만 채워서 적재** (sparse drilldown 데이터 보존). 실측 비율: rep_with_summary ≈ 0.23 (Step 7 첫 적재 기준).
...
4. ⚠ `assign_trend_cluster.py` partial 매칭 — `multiplier_for_n(1/2/3)` 함수 구현됨, 단 `build_contributions` 는 **항상 N=3 만 호출** (정책 미진입). 후속 phase 에 partial 적재 정책 확장 시 활성화. 현재 ItemDistribution top-1 G/T/F → exact_key 매칭만. **별개로 share-weighted summary path 는 α/β1/β2 클로즈 (2026-04-28)** — `_build_contexts` G×T×F cross-product fan-out + N<3 zero contribution + mass preservation invariant. β3 (group_by_cluster) / β4 (drilldown) / γ (history schema 마이그) 미진입. 자세한 내용 §2.4 share-weighted summary path 진척 참조
