...
추출 방법: **텍스트로는 잘 안 잡힘.** 캡션에 "pastel mint"라고 쓰는 경우는 드묾. 대부분 이미지를 봐야 알 수 있는 속성. → **Instagram 이미지에서만 VLM으로 추출. YouTube에서는 컬러 추출 안 함** (한 영상에 여러 옷이 나와서 프레임별 추출 비현실적).
...
YouTube 영상에서는 컬러 추출 안 함 (한 영상에 여러 옷이 나와서 프레임별 추출 비현실적).
...
- YouTube 영상에서 컬러 추출 → 프레임별 추출 비현실적. 컬러는 Instagram 이미지만.
- YouTube 썸네일 VLM 분석 → 제목+설명+태그로 충분
- YouTube 영상 프레임별 분석 / 음성 텍스트 변환(ASR) → PoC에서 시도
