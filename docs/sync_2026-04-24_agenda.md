...
### 1.6 TSV 로더 실측 반영 (2026-04-22 추가 — loaders/tsv_raw_loader.py)

fixture + 실 TSV 4건을 `TsvRawLoader` 로 변환하며 발견된 가정 / Open Question:

| 항목 | 현재 가정 | 크롤러 팀 확인 필요 |
|---|---|---|
| `post_id` | TSV col [2] ULID (`01KPNKJ80633...`) | BE/DW 팀이 primary-key 로 **Instagram shortcode (`DXUTMtzAjAj`)** 를 기대하는지 vs ULID 로 OK 인지 |
| `source_type` | `posting.tsv` 전부 `INFLUENCER_FIXED`, `hashtag_search` 는 `HASHTAG_TRACKING` | TSV 에 source_type 컬럼을 추가해서 혼재 케이스 (influencer × hashtag overlap) 명시할지 |
| `image_urls` | posting 은 blob path (col [11]), hashtag_search 는 CDN URL (col [5]) | canonical 하나로 통일 필요 — 둘 다 유지 vs CDN 표준화 |
| `hashtag_search.account_handle` | **None** 로 contract 완화 (`str | None = None`) | 크롤러가 shortcode 로 post detail 추가 쿼리해 handle 채울 수 있는지 |
| `hashtag_search.post_date` | **placeholder 2026-04-15** (TSV 에 없음) | 크롤러가 실 post_date 추가 수집해주기로 (사용자 요청 — 4/24 전) |
| `hashtag_search` likes/comments | 0 (TSV 에 0 으로 찍힘) | 해시태그 검색 endpoint 의 인게이지먼트 수집이 의도인지 누락인지 |
| `youtube video_id` | URL 에서 `v=` regex 추출 (TSV col [4]) ≠ ULID (col [2]) | OK |
| `youtube blob video path` | TSV col [16] 존재하나 우리 contract 에 video_path 필드 없음 → 버림 | M3 Blob 연동 시 필요하면 contract 확장 |
...
- 영향: 기존 sample JSON fixture 영향 없음 (str 제공). top_influencers 계산 시 None 필터 필요 (M3)

### 1.7 raw contract shape (우리 `contracts/raw.py`) 정합
