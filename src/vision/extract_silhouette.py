...
Case 1: 미분류 포스트의 silhouette 를 color_extractor 로 추출.
Case 2: 클러스터 palette 보강과 함께 silhouette 분포 업데이트.

이 모듈은 vision.color_extractor 의 Protocol/함수를 재사용한다 (별도 client 만들지 않는다).
"""
...
def extract_silhouettes(
    ...
    results: list[ColorExtractionResult] = run_color_extraction(items, extractor, cap=cap)
