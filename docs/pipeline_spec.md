...
4. ⚠ `assign_trend_cluster.py` partial 매칭 — `multiplier_for_n(1/2/3)` 함수 구현됨, 단 `build_contributions` 는 **항상 N=3 만 호출** (정책 미진입). 후속 phase 에 partial 적재 정책 확장 시 활성화. 현재 ItemDistribution top-1 G/T/F → exact_key 매칭만
